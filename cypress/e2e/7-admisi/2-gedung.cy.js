...
describe('Manajemen Gedung', () => {
    ...
    it.only('Searching for a building name', () => {
        cy.fixture("login-with-valid-credential").then(user => {
            ...
            cy.get('.px-open > .px-nav-dropdown-menu > :nth-child(2) > .xhr > .px-nav-label').click() // klik sub menu Gedung
            ...
            cy.get('.table-common-even > :nth-child(5)').and('contain', 'Rendra')            
        })
    })

    // Menambahkan gedung baru
    it('Adding a building name', () => {
        cy.fixture("login-with-valid-credential").then(user => {
            const username = user.username
            const password = user.password
    ...
    it('Edit a building name', () => {
        cy.fixture("login-with-valid-credential").then(user => {
            ...
            cy.get('.panel-title').and('contain', 'Ubah Gedung') //assertion memuat tulisan Ubah Gedung
    ...
    it('Delete a building data', () => {
        cy.fixture("login-with-valid-credential").then(user => {
            ...
            cy.get('#dialog_delete > h2').should('have.text', 'Menghapus Manajemen Gedung') // assertion

...
describe('Manajemen Penerimaan', () => {
    ...
    it.only('Opens Penerimaan page', () => {
        cy.fixture("login-with-valid-credential").then(user => {
            ...
            cy.get('.px-open > .px-nav-dropdown-menu > :nth-child(7) > .xhr > .px-nav-label').click() // klik sub menu Setting Pengumuman
            cy.get('h1').and('contain', 'Manajemen Penerimaan') // assertion bahwa elemen h1 memuat tulisan Setting Pengumuman
        })
    })
